...
const GlitchText = ({ text, className, goldEffect = false }: GlitchTextProps) => {
  return (
    <GlitchEffect type="text" className={cn(
      ...
      {goldEffect && (
        <>
          <span className="absolute top-0 left-0 opacity-60 text-amber-600">{text}</span>
          <span className="absolute top-0 left-0 opacity-60 text-amber-400">{text}</span>
        </>
      )}
    </GlitchEffect>
  );
